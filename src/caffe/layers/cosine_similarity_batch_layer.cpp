...
template <typename Dtype>
void CosineSimilarityBatchLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  size_t dim =
      bottom[0]->num() > 1 ? bottom[1]->num() * (bottom[1]->num() - 1) / 2 : 1;

  top[0]->Reshape(dim, 1, 1, 1);
...
void CosineSimilarityBatchLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  caffe_cpu_gemm(CblasNoTrans, CblasTrans, bottom[0]->num(), bottom[0]->num(),
                 bottom[0]->channels(), Dtype(1.), bottom[0]->cpu_data(),
                 bottom[0]->cpu_data(), Dtype(0.), xy_blob.mutable_cpu_data());

  const Dtype* xy_ = xy_blob.cpu_data();

  int pos_label =
      this->layer_param_.cosine_similarity_batch_param().pos_label();
  int neg_label =
      this->layer_param_.cosine_similarity_batch_param().neg_label();

  int k = 0;
  for (int i = 0; i < bottom[0]->num(); ++i) {
    for (int j = i + 1; j < bottom[0]->num(); ++j) {
      top[0]->mutable_cpu_data()[k] =
          xy_[i * bottom[0]->num() + j] /
          sqrt(xy_[i * bottom[0]->num() + i] * xy_[j * bottom[0]->num() + j]);
...
template <typename Dtype>
void CosineSimilarityBatchLayer<Dtype>::Backward_cpu(
  ...
  for (int i = 0; i < num; ++i) {
    for (int k = 0; k < channels; ++k) {
      ...
      for (int j = 0; j < num; ++j) {
        ...
        Dtype add_dsdx =
            top[0]->cpu_diff()[h] *
            (bottom[0]->cpu_data()[j * channels + k] / sqrt(xy_ii * xy_jj) -
             xy_ij / xy_ii * bottom[0]->cpu_data()[i * channels + k] /
                 sqrt(xy_ii * xy_jj));
        dsdx += add_dsdx;
      }
