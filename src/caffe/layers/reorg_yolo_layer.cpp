...
void ReorgYoloLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ReorgYoloLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                        const vector<Blob<Dtype>*>& top) {
    ...
    for (int ic = 0; ic < channels; ++ic) {
      for (int iy = 0; iy < height; ++iy) {
        for (int ix = 0; ix < width; ++ix) {
          int inIndex = ix + iy * width + ic * width * height +
                        ib * width * height * channels;
          ...
          int outIndex = ow + oh * width * stride_ +
                         oc * width * stride_ * height * stride_ +
                         ib * width * stride_ * height * stride_ * out_channels;
