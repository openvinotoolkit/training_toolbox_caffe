...
namespace caffe {
  ...
  void PSROIPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
      // LOG(INFO) << "PSROIPoolingLayer::Forward_cpu";
      const Dtype* bottom_data_begining = bottom[0]->cpu_data();
      const Dtype* bottom_rois_begining = bottom[1]->cpu_data();
      Dtype* top_data = top[0]->mutable_cpu_data();
      int* mapping_channel_ptr = mapping_channel_.mutable_cpu_data();
      int count = top[0]->count();
      for (int i = 0; i < count; ++i) {
        top_data[i] = Dtype(0);
        mapping_channel_ptr[i] = -1;
      }
      // LOG(INFO) << "rois num: " << bottom[1]->num();

      for (int index = 0; index < count; ++index) {
        // LOG(INFO) << "output index: " << index << " / " << count;
        // The output is in order (n, ctop, ph, pw)
        int pw = index % pooled_width_;
        int ph = (index / pooled_width_) % pooled_height_;
        int ctop = (index / pooled_width_ / pooled_height_) % output_dim_;
        int n = index / pooled_width_ / pooled_height_ / output_dim_;
        // LOG(INFO) << "output coords: " << n << "," << ctop << "," << ph << "," << pw;

        // [start, end) interval for spatial sampling
        const Dtype* bottom_rois = bottom_rois_begining + n * 5;
        ...
        const Dtype* bottom_data = bottom_data_begining + ((roi_batch_ind * channels_ + c) * height_ * width_);
        Dtype out_sum = 0;
