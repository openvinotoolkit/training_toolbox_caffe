...
void ReLU6Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < count; ++i) {
    top_data[i] = std::min(std::max(bottom_data[i], Dtype(0)), Dtype(n));
  }
}
...
void ReLU6Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    const int count = bottom[0]->count();
    Dtype n = this->layer_param_.relu6_param().n();
    for (int i = 0; i < count; ++i) {
      bottom_diff[i] = top_diff[i] * (bottom_data[i] > 0) * (bottom_data[i] < n);
    }
  }
}
