...
void RegionYoloLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                     const vector<Blob<Dtype>*>& top) {
  const int start_axis =
      bottom[0]->CanonicalAxisIndex(this->layer_param_.flatten_param().axis());
...
void RegionYoloLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                        const vector<Blob<Dtype>*>& top) {
  RegionYoloParameter region_yolo_param =
      this->layer_param_.region_yolo_param();
...
template <typename Dtype>
void RegionYoloLayer<Dtype>::softmax(const Dtype* input, int classes,
                                     int stride, Dtype* output) {
...
template <typename Dtype>
void RegionYoloLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                         const vector<Blob<Dtype>*>& top) {
  ...
  for (int b = 0; b < batch; ++b) {
    for (int n = 0; n < num_; ++n) {
      int index = entry_index(width, height, coords_, classes_, inputs, b,
                              n * width * height, 0);
      for (int i = index; i < index + 2 * width * height; ++i) {
        top_data[i] = logistic_activate(top_data[i]);
      }

      index = entry_index(width, height, coords_, classes_, inputs, b,
                          n * width * height, coords_);
  ...
  int index =
      entry_index(width, height, coords_, classes_, inputs, 0, 0, coords_ + 1);

  int batch_offset = inputs / num_;
  int groups = width * height;
  int group_offset = 1;
  int stride = width * height;
  for (int b = 0; b < batch * num_; ++b) {
    for (int g = 0; g < groups; ++g) {
      softmax(bottom_data + index + b * batch_offset + g * group_offset,
              classes_, stride,
              top_data + index + b * batch_offset + g * group_offset);
    }
  }
}
