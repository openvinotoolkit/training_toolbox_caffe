...
def write_xml(image, image_name, bboxes, path_out, show, root):
    """
    Write to xml file
    """
    global object_num, image_num
    ...
    for bbox in bboxes:
        ...
        xmax = bbox[2]
        ymax = bbox[3]
        obj = ET.SubElement(image_elem, "object{:0=6}".format(object_num))
        ET.SubElement(obj, "type").text = "face"
        ET.SubElement(obj, "id").text = str(object_num)
        ET.SubElement(obj, "bbox").text = "{} {} {} {}".format(xmin, ymin, xmax-xmin, ymax-ymin)
        object_num += 1
...
def main():
    """
    Convert original WIDERFace annotation to xml format
    """
    args = parse_args()
    ...
    while idx < len(lines):
        ...
        write_xml(image, image_name_out, bboxes, path_out, show, root)
