...
def python_net_file():
    with tempfile.NamedTemporaryFile(mode='w+', delete=False) as f:
        f.write("""name: 'center_loss_layer_test_net'
                   force_backward: true
                   ...
                   layer { name: "anchor2" type: "Input" top: "anchor2"
                           input_param { shape { dim: 2 dim: 5 dim: 3 dim: 4 } } }
                   layer { type: 'Python' name: 'center_loss' bottom: 'detections' bottom: 'centers'
                           bottom: 'anchor1' bottom: 'anchor2' top: 'center_loss_value'
                           python_param { module: 'caffe.custom_layers.center_loss_layer' layer: 'CenterLossLayer'
...
class TestCenterLossLayer(unittest.TestCase):
    def setUp(self):
    ...
    def test_forward(self):
        ...
        for i in range(3):
            ...
            dist = 1.0 - np.sum(self.centers[class_id] * self.anchors[anchor_id][item, :, y_pos, x_pos])
        ...
        predicted_loss_value = net_outputs['center_loss_value']
