...
def plot_curves(curves):
    colors = sns.color_palette("hls", len(curves.keys()))
    plt.style.use("ggplot")
    plt.figure()
    i = 0
    for curve_name, curve_points in curves.iteritems():
        if len(curve_points[0]) > 1:
            plt.plot(curve_points[1], curve_points[0], linewidth=2, label=curve_name, color=colors[i])
            i += 1
        else:
            print(curve_points)
            x, y = curve_points
    ...
    y_range = np.concatenate((np.array([0.03, 0.05]), np.arange(0.1, 0.55, 0.1), np.array([0.64, 0.8, 1])))
    plt.yscale("log", nonposy="clip", subsy=[])
    plt.yticks(y_range, map(lambda i: "{:.2f}".format(i), y_range))
...
def plot_pr_curves(curves):
    ...
    for curve_name, curve_points in curves.iteritems():
        if len(curve_points[0]) > 1:
            ...
            print(curve_points)
            plt.plot(curve_points[1], curve_points[0], linewidth=2, marker='D', markersize=10, label=curve_name)
    plt.xlim(-0.05, 1.05)
...
def parse_args():
    ...
    parser.add_argument("--v", "--visible", dest="is_visible_subset", action="store_true",
                        help="Apply filter to ground truth: delete bounding boxes with visible tags other than visible.")
    ...
    parser.add_argument("--im", "--imsize", dest="image_size", nargs=2, type=int, default=(1920, 1080),
                        help="Image resolution. Used for filtering.")
    parser.add_argument("--obj", "--objsize", dest="object_size", nargs=2, type=int, default=(10, 600),
    ...
    parser.add_argument("--mm", "--multiple_matches", dest="allow_multiple_matches", action="store_true",
...
def reasonable_filter(annotation, height_range=(0, sys.maxint), image_size=(1920, 1080), class_lbl='pedestrian', treat_as_square=False):
    for image_annotation in annotation.values():
        ...
        for ignore_reg in image_annotation.ignore_regs:
            image_annotation.objects.append({"is_ignored": True,
                                             "bbox": [ignore_reg[0], ignore_reg[1], ignore_reg[0] + ignore_reg[2],
                                                     ignore_reg[1] + ignore_reg[3]]})

    return annotation


def filter_annotation(annotation, height_range=(0, sys.maxint), image_size=(1920, 1080), treat_as_square=False):
    for key, image_annotation in annotation.iteritems():
        for object_annotation in image_annotation:
...
if __name__ == "__main__":
    ...
    transformers = {"id": int, "bbox": lambda x: map(float, x.strip().split()), "score": float,
    ...
    images_with_gt = set([image_annotation.image_path for image_annotation in ground_truth.itervalues()])

    for image_annotation in ground_truth.values():
        for object_annotation in image_annotation:
            object_annotation = convert_object_info(transformers, object_annotation)

    if args.is_reasonable_subset:
        ground_truth = reasonable_filter(ground_truth, args.object_size, args.image_size, args.class_lbl, args.treat_as_square)
    ...
    for detections_file_path in args.detections_file_path:
        ...
        recall, precision, miss_rates, fppis = evaluate_detections(ground_truth.values(),
                                                                   detections.values(),
                                                                   args.class_lbl,
                                                                   allow_multiple_matches_per_ignored=
                                                                   args.allow_multiple_matches,
        ...
        if args.result_file_path:
            results = {"miss_rates": miss_rates,
                       ...
                       "ap": ap,
                       "mr": mr}
            with file(args.result_file_path, "w") as f:
