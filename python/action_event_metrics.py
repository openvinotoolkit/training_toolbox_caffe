...
NET_OUTPUTS_MAP = {0: 0, 1: 1, 2: 2, 3: 0, 4: 0, 5: 0}
...
def load_annotation(annotation_path, video_height, video_width):
...
def extract_detections(image, net, in_blob_name, out_blob_name, min_det_conf, min_action_conf):
    ...
    all_detections = [BBoxDesc(id=-1,
                               label=NET_OUTPUTS_MAP[det_label[i]],
                               det_conf=det_conf[i],
...
def main():
    ...
    for task_id, task in enumerate(tasks):
        ...
        annotation = load_annotation(task[0], video_height, video_width)
        ...
        track_events = process_tracks(tracks, window_size, min_action_length)
