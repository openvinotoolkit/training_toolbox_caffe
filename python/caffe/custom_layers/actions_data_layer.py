...
BBox = namedtuple('BBox', 'track_id, action, xmin, ymin, xmax, ymax, occluded')
...
class SampleDataFromDisk(object):
    ...
    def __init__(self, task_path, ignore_occluded, action_names_map, valid_action_ids, ignore_class_id):
        ...
        :param action_names_map: Dictionary to map action names to ID
        :param valid_action_ids: List of valid action IDs
        :param ignore_class_id: ID of ignored class
        """

        self._action_names_map = action_names_map
        ...
        self._all_frames = []
        self._class_queues = {i: [] for i in self._valid_action_ids if i != self._ignore_class_id}
        glob_class_counts = {i: 0 for i in self._valid_action_ids if i != self._ignore_class_id}
        ...
        for task_id, task in enumerate(tasks):
            ...
            for frame_id in annotation:
                ...
                local_class_counts = {i: 0 for i in self._valid_action_ids if i != self._ignore_class_id}
                for gt_object in gt_objects:
                    class_id = gt_object.action
    ...
    def _read_annotation(annotation_path, image_size, ignore_occluded,
                         action_names_map, valid_action_ids, id_shift):
        ...
        for track in root:
            ...
            for bbox in track:
                ...
                if action_name_id not in valid_action_ids:
                    continue
...
class ActionsDataLayer(BaseLayer):
    ...
    def _objects_to_blob(item_id, objects):
        ...
        objects_annotation = []
        for bbox in objects:
            object_desc = [float(x) for x in [item_id, bbox.action, bbox.track_id,
    ...
    def _sample_params(self, image, objects_list):
        ...
        def _rand_bbox():
            ...
            return BBox(track_id=-1, action=-1,
        ...
        action_prob = np.random.uniform(0.0, 1.0)
        do_expand = self.spatial_transforms_probs_[0] <= action_prob < self.spatial_transforms_probs_[1]
        do_crop = self.spatial_transforms_probs_[1] <= action_prob < self.spatial_transforms_probs_[2]
        ...
        if do_crop:
            valid_objects = [obj for obj in objects_list if obj.action != self._ignore_class_id]
