#ifndef CAFFE_PSROI_ASSEMBLING_LAYER_HPP_
#define CAFFE_PSROI_ASSEMBLING_LAYER_HPP_
 ...
 * specified by input, takes
 *        as input N position-sensitive score maps and a list of R regions of
 * interest.
 *   ROIPoolingLayer takes 2 inputs and produces 1 output. bottom[0] is
 *   [N x (C x K^2) x H x W] position-sensitive score maps on which pooling is
 * performed. bottom[1] is
 ...
 *   of ROI rectangle (including its boundaries). The output top[0] is [R x C x
 * K x K] score maps pooled
 *   within the ROI tuples.
 * @param param provides PSROIAssemblingParameter psroi_pooling_param,
 *        with PSROIAssemblingLayer options:
...
class PSROIAssemblingLayer : public Layer<Dtype> {
 public:
  explicit PSROIAssemblingLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {
  }
 ...
 protected:
  ...
  //  int output_dim_;
  int input_channels_num_;
  int output_channels_num_;
  int group_size_;
...
#endif  // CAFFE_PSROI_ASSEMBLING_LAYER_HPP_
